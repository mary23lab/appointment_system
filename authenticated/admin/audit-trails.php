<?php
    include 'session.php'; 
    $audit_trail = $conn->query("SELECT audit_trail.*, users.*, audit_trail.id AS at_id FROM audit_trail INNER JOIN users ON audit_trail.user_id = users.id ORDER BY audit_trail.id DESC");
?>
                    ...
                    <table class="table table-sm table-bordered table-striped data-table small">
                        ...
                        <tbody>
                            <?php while($a = $audit_trail->fetch_array()) : ?>
                                <tr>
                                    <td style="padding-top: 8px"><?= $a['at_id']; ?></td>
                                    <td style="padding-top: 8px"><?= $a['username']; ?> <?= $a['user_id'] == $userId ? '<i>(Me)</i>' : ''; ?></td>
