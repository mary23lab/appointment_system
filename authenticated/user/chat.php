...
<?php 
    $messages = $conn->query("SELECT `chat_messages`.*, `users`.*, `chat_messages`.`id` AS cmid FROM `chat_messages` INNER JOIN `users` ON `chat_messages`.`sender` = `users`.`id` WHERE `chat_messages`.`user_id`=$userId ORDER BY `chat_messages`.`created_at` ASC");
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if($message !== "" || $_FILES['image']['tmp_name'] !== ""){
            ...
            $conn->query("INSERT INTO chat_messages (user_id, message, sender, image_path) VALUES ('$userId', '$message', '$userId', '$image')");
            mysqli_close($conn);
        }
        ...
        <div class="container-fluid">
            <div class="row mb-3">
                <div class="col-sm-12 d-flex align-items-center" style="font-size: 18px">
                    ...
                    <strong class="mt-1">Chat w/ Admin</strong>
                </div>
            </div>
        </div>
    </section> 
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-xl-6">
                                        <div class="d-flex justify-content-end align-items-center">
                                            <a href="chat.php?method=delete_chat&id=<?= $m['cmid']; ?>">
